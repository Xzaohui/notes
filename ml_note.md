...
Cα(Tt)=C(Tt)+α|Tt|
$$
其中C(Tt)为误差（例如基尼指数）,∣Tt∣为子树的叶节点数，α > = 0为参数，用来权衡训练数据的拟合程度和模型的复杂度。
...
现在考查逻辑斯谛回归模型的特点。一个事件的几率 （odds）是指该事件发生的概率与该事件不发生的概率的比值。逻辑斯谛回归是对数几率中的线性回归，如果事件发生的概率是p，那么该事件的几率是p/1-p，该事件的对数几率 （log odds）或 logit 函数是:
...
### GBDT常用正则化方法
GBDT 有非常快的降低 Loss 的能力，这也会造成一个问题：Loss 迅速下降，容易使模型低 variance ，高 bias，造成过拟合。
