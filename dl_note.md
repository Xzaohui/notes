...
NCE loss，本身起到的作用就是"**拉近正样本对之间距离，推远负样本对之间距离**"

NCE损失和softmax结构很像，带有温度系数。
...
经常应用与回归问题，输出为连续值，最后一层没有softmax。
...
而我们在pytorch中使用的交叉熵损失函数一般只关注正标签，因此p（x）=1，形式如下:

$loss=-\sum_{x=label_x}\log q(x)$
...
p(x)是0，1标签，q(x)是经过softmax后的RNN输出的概率矩阵。
...
### 非零均值函数
在某一次反向传播时，对于神经元A来说，w1、w2…改变的方向是统一的，或正或负，导致收敛慢。

![](https://img-blog.csdnimg.cn/2020091917020164.png#pic_center)
...
只有少数神经元会被激活，大部分神经元会被置为 0，有稀疏性，信息耦合程度降低，降低训练难度和过拟合现象。
...
$$
SELU 激活能够对神经网络进行自归一化（self-normalizing），selu激活函数通过设置两个特殊的参数α和λ，经过该激活函数后使得样本分布自动归一化到0均值和单位方差
...
1. 定义一个 $c_{t}$ 向量, 是**所有先前解码器时间步的注意力分布之和**， 此时的 $c_{t}$ 向量代表了先前步 已经覆盖到的原文单词。
$$
